...
//         // 2️⃣ Decrease stock in products
...
export function OrderProvider({ children }) {
    ...
    // ✅ Grab setProducts too
    const { products = [], setProducts } = useContext(DashContext);
    ...
    //  useEffect(() => {
    //     localStorage.removeItem("orders"); // or localStorage.clear() if you want everything cleared
    // }, []);
    ...
    // ✅ UPDATED handleAddToCart to decrease stock globally
    // const handleAddToCart = (item) => {
    //     const size = selectedSize[item.name] || "S";
    //     const price = Number(item.sizes?.[size] || 0);
    ...
    //     // 2️⃣ Decrease stock in DashContext
    //     const updatedProducts = products.map((p) => {
    //         if (p.id === item.id) {
    //             return { ...p, stock: Number(p.stock) - 1 };
    //         }
    ...
    const handleAddToCart = (item) => {
        ...
        // Reset success/payment if user starts a new order
        setShowSuccess(false);
        setShowReceipt(false);
        setShowPayment(false);
    ...
    const confirmOrder = (paymentMethod, customerName) => {
        ...
        const updatedProducts = products.map((p) => {
            const cartItem = cart.find(
                (c) => c.name === p.name && c.size === (selectedSize[p.name] || "S")
            );
        ...
        setOrders(prev => {
            ...
            localStorage.setItem("orders", JSON.stringify(updatedOrders)); // ✅ only here
    ...
    const onClose = () => setShowOrder(false);
